...
public class TestCraps
{
	...
	{
		// game = new CrapsGame();
		// game = new CrapsGame(new Die(), new Die());  //49.247%
		// game = new CrapsGame(new CrookedDie1(), new Die()); // 58.3558%
		// game = new CrapsGame(new CrookedDie1(), new CrookedDie1());  //100%
		
		// game = new CrapsGame(new CrookedDie2(), new Die());   //48.3428%
		
		game = new CrapsGame(new CrookedDie1(), new CrookedDie2());    //0%
	...
	public void reportStats()
	{
		...
		for (int numSteps = 1; numSteps < winForNumberOfStepsOf.length; numSteps++)
		{
			StdOut.println("(wins,losses) ending at " + numSteps + " roll" + ((numSteps > 1) ? "s" : "") + ": ("
					+ winForNumberOfStepsOf[numSteps] + "," + lossSteps[numSteps] + ")");
		}
